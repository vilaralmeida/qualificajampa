...
#' @examples
#' avaliacaoTotalDadosAbertosJP(): Retorna data.frame com painel de Qualificação de Dados Abertos de Joao Pessoa
...
avaliacaoTotalDadosAbertosJP <- function(anomaly_threshold = 0.8) {
  ...
  df <- getTabelaDadosTotal(obj = get(tabelas[1]),
                                   tabela = tabelas[1],
                                   categoria = categorias[1],
                                   anomaly_threshold = anomaly_threshold)

  for (i in 2:length(tabelas)) {
    temp <- getTabelaDadosTotal(obj = get(tabelas[i]),
                                     tabela = tabelas[i],
